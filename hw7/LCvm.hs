...
-- For Homework 7
        DUP   ->  if (sp == 0 ) then (pc+1, 0, set stk sp (regs!arg), regs) else  (pc+1, sp+1, set stk sp top, regs)   --fixMe "DUP VM instruction"
...
-- For Homework 7
        INC   -> (pc' , sp  , stk                        , regs)
            where pc' = mod (pc+arg+1) 256 --fixMe "INC VM instruction"
-- For Homework 7
        DEC   -> (pc' , sp  , stk                        , regs)
            where pc' = mod (pc+arg-1) 256  -- "DEC VM instruction"
        JMP   -> (pc' , sp  , stk                        , regs)
            where pc' = pc+arg+1
        JZ    -> (pc' , sp-1, stk                        , regs)
        ...
        JNZ   -> (pc' , sp-1, stk                        , regs)
            where pc' = if top/=0 then (pc+arg+1) else (pc+2)
-- For Homework 7
        CALL  -> (arg , sp+1  ,set stk (pc'+2) (nxt*arg)         , regs)
            where pc' = pc+1 --fixMe "CALL VM instruction"
-- For Homework 7
        RET   -> (top , sp+1  ,set stk (pc'+2)  (nxt*top)        , regs)
            where pc' = sp-1 --fixMe "CALL VM instruction"
...
stepRun ap = mapM_ showStep (zip [1..] trace)
    where
      ...
      showStep (n,c)         -- print a numbered config and read a char
            = do { putStr "Step "; print n ; prn obj c ; getChar }
...
underOver asms = fixMe "underOver"
